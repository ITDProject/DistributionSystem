...
def initDSO(DSODict):
	...
	print('IDSO Agent', market['name'],'is Started', flush = True)
...
def initialize():
	...
	global RealPower, json_data, DALMP, Dsystem, distribution_load, hvac_load, controllerData, house_names
	global fncs_publish
	...
	for i in range(len(P_sorted)):
		...
		if LoadMet >= MaxON:
			if diff1 < diff2:
				...
				CP = Pi_sorted[i]
			print('Retail price(s) is/are set to:', CP, flush = True)
			CP = round(CP,2)
			if CP >= 0:
				CPVector[0] = CP
				CPVector[1] = 0
				...
				CPVector[1] = CP
			print('Retail price(s) is/are set to:', CPVector[0], CPVector[1], flush = True)
			return CPVector
	CP = round(CP,2)
	if CP >= 0:
		CPVector[0] = CP
...
with warnings.catch_warnings():
	...
	while ts <= tmax:
		...
		for key in events:
			...
			if title.startswith('DALMP'):
				print('printing DALMP value vector ', value, flush = True)
				DALMP_temp = value.split(',')
				for i in range(NHour):
					DALMP[i] = get_number(DALMP_temp[i])
				print('ts_dam: ',ts, 'printing DALMP: ', DALMP, flush = True)

			if title.startswith('RTLMP'):
				RTLMP = get_number(value)
				print('ts_rtm: ',ts, 'printing RTLMP: ', RTLMP, flush = True)
		...
		if prev_day != day and day >= 1:
			...
			outfile = open('OutputFiles/PlotFiles/plotdata' + '_D' + str(day-1) + '.json', 'w') 
